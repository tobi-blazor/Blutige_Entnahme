...
import { MaterialIcons } from '@expo/vector-icons'; // Import von MaterialIcons

// Funktion, die basierend auf einem 4-stelligen String vier Icons generiert
const generateIcons = (input: string): React.ReactNode[] => {
  ...
  const firstThreeLetters = input.substring(0, 3);
    let iconName: keyof typeof MaterialIcons.glyphMap; // Typ für Icon-Namen
  ...
  for (let i = 3; i < 6; i++) {
    // Ordne jeder Ziffer ein Icon zu
    switch (input[i]) {
      case '0':
        iconName = 'circle';
        break;
    ...
    icons.push(<MaterialIcons key={i-2} name={iconName} size={30} color="black" />);
  }
